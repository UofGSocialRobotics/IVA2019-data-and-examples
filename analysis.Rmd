...
get_avg_participants_per_condition <- function(data1, data2, data3) {
  return(mean(mean(data1$Total)/2, mean(data2$Total)/2, mean(data3$Total)/2))
}
...
pwr.anova.test(k=3, n=20, sig.level=0.05, f=0.25)
```
Zoinks that ain't great. We need to up our sample size! 
```{r}
pwr.anova.test(k=3, sig.level=0.05, f=0.25, power=0.8)
```
To *52* participants per group (survey) the look of it! 
